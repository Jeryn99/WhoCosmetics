...
public class WardrobeBlock extends Block {
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity entity, Hand hand, BlockRayTraceResult hit) {
        ...
        WardrobeTile wardrobeTile = (WardrobeTile) world.getTileEntity(pos);
        int x = pos.getX();
        int y = pos.getY();
        int z = pos.getZ();
            ...
            if(entity.isSneaking()) {
                NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
                    ...
                    public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
                        return new WardrobeContainer(id, inventory, new PacketBuffer(Unpooled.buffer()).writeBlockPos(new BlockPos(x, y, z)));
                    }
                }, new BlockPos(x, y, z));
            } else {
                wardrobeTile.setOpen(!wardrobeTile.isOpen());
                world.playSound(null, pos.getX()+0.5D, (double)pos.getY() + 0.5D, pos.getZ()+0.5D, wardrobeTile.isOpen() ? SoundEvents.BLOCK_CHEST_OPEN : SoundEvents.BLOCK_CHEST_CLOSE, SoundCategory.BLOCKS, 0.5F, world.rand.nextFloat() * 0.1F + 0.9F);
                wardrobeTile.markDirty();
            }
